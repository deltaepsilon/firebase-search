  ...
  this.ensureExistingUser = function() {
    ref.orderByKey().limitToLast(1).once('value', function(snap) {
      if (!snap.numChildren()) {
        ...
        fakeRef.set(true).then(function() {
          fakeRef.remove();
        });
      }
    });
...
function setPrototype(firebaseSearch) {
  ...
  firebaseSearch.elasticsearch = {
    indices: {
      exists: function(params) {
        ...
        return toPromise(
          firebaseSearch.elasticsearch.client.indices.exists.bind(firebaseSearch.elasticsearch.client.indices),
          params
        );
      },
      delete: function(params) {
        ...
        return toPromise(
          firebaseSearch.elasticsearch.client.indices.delete.bind(firebaseSearch.elasticsearch.client.indices),
          params
        );
      },
      create: function(params) {
        ...
        return toPromise(
          firebaseSearch.elasticsearch.client.indices.create.bind(firebaseSearch.elasticsearch.client.indices),
          params
        );
      },
      ensure: function() {
        ...
        return firebaseSearch.elasticsearch.indices.exists().then(function(exists) {
          return exists || firebaseSearch.elasticsearch.indices.create(params);
        });
      }
    },
    ping: function() {
      return toPromise(firebaseSearch.elasticsearch.client.ping.bind(firebaseSearch.elasticsearch.client), {
        hello: 'elasticsearch!'
      });
    },
    create: function(params) {
      ...
      return firebaseSearch.elasticsearch
        .exists(params)
        ...
        .then(function() {
          return toPromise(
            firebaseSearch.elasticsearch.client.create.bind(firebaseSearch.elasticsearch.client),
            params
          );
        });
    },
    ...
    firebase: {
      ...
      build: function(returnPromise) {
        return firebaseSearch.getLastKey().then(function(lastKey) {
          return new Promise(function(resolve, reject) {
            ...
            var total = 0;
            var handler = function(snap) {
              var promise = firebaseSearch.elasticsearch
                .create({
                  id: snap.key,
                  body: snap.val()
                })
        ...
        return new Promise(function(resolve, reject) {
          ...
          firebaseSearch.elasticsearch.handlers = {
            child_added: function(snap) {
              if (!started) {
                // Skip the first child_added event. It's often an existing record.
                started = true;
                resolve(snap.key);
              } else {
          ...
          firebaseSearch.elasticsearch.listeningRefs.child_added.once('value').then(function(snap) {
            if (!snap.numChildren()) (started = true), resolve(true);
            firebaseSearch.elasticsearch.listeningRefs.child_added.on(
              'child_added',
              firebaseSearch.elasticsearch.handlers.child_added
            );
            firebaseSearch.elasticsearch.listeningRefs.child_changed.on(
              'child_changed',
              firebaseSearch.elasticsearch.handlers.child_changed
            );
            firebaseSearch.elasticsearch.listeningRefs.child_removed.on(
              'child_removed',
              firebaseSearch.elasticsearch.handlers.child_removed
            );
          });
        });
      },
      stop: function() {
        if (firebaseSearch.elasticsearch.handlers && firebaseSearch.elasticsearch.listeningRefs) {
          firebaseSearch.elasticsearch.listeningRefs.child_added.off(
            'child_added',
            firebaseSearch.elasticsearch.handlers.child_added
          );
          firebaseSearch.elasticsearch.listeningRefs.child_changed.off(
            'child_changed',
            firebaseSearch.elasticsearch.handlers.child_changed
          );
          firebaseSearch.elasticsearch.listeningRefs.child_removed.off(
            'child_removed',
            firebaseSearch.elasticsearch.handlers.child_removed
          );
        } else {
  ...
  firebaseSearch.algolia = {
    ...
    clearIndex: function(shouldWait) {
      return toPromise(
        firebaseSearch.algolia.index.clearIndex.bind(firebaseSearch.algolia.index),
        undefined,
        shouldWait
      );
    },
    ...
    exists: function(name) {
      ...
      return firebaseSearch.algolia.listIndexes().then(function(indexes) {
    ...
    firebase: {
      build: function() {
        return firebaseSearch.getLastKey().then(function(lastKey) {
          ...
          return new Promise(function(resolve, reject) {
            var ref = firebaseSearch.ref.orderByKey();
            const handler = ref.on('child_added', function handleChildAdded(snap) {
              var obj = snap.val();
              obj.objectID = snap.key;

              total++;
              queue.push(function(done) {
                firebaseSearch.algolia.addObject(obj, true).then(() => successful++).catch(() => failed++).then(done);
              });
              if (snap.key == lastKey) {
        ...
        return new Promise(function(resolve, reject) {
          ...
          firebaseSearch.algolia.handlers = {
            child_added: function(snap) {
              if (!started) {
                // Skip the first child_added event. It's often an existing record.
                const index = startingKeys.indexOf(snap.key);
                startingKeys.splice(index, 1);
                ...
                var obj = snap.val();
                obj.objectID = snap.key;
                firebaseSearch.algolia.addObject(obj, true).then(function() {
            ...
            child_changed: function(snap) {
              var obj = snap.val();
              obj.objectID = snap.key;
              firebaseSearch.algolia.saveObject(obj, true).then(function() {
                // firebaseSearch.log('algolia_child_changed', snap.key);
                firebaseSearch.fire('algolia_child_changed', obj);
              });
            },
            child_removed: function(snap) {
              firebaseSearch.algolia.deleteObject(snap.key, true).then(function() {
          ...
          firebaseSearch.algolia.listeningRefs.child_added.once('value').then(function(snap) {
            ...
            firebaseSearch.algolia.listeningRefs.child_changed.on(
              'child_changed',
              firebaseSearch.algolia.handlers.child_changed
            );
            firebaseSearch.algolia.listeningRefs.child_removed.on(
              'child_removed',
              firebaseSearch.algolia.handlers.child_removed
            );
          });
        });
      },
      stop: function() {
        if (firebaseSearch.algolia.handlers && firebaseSearch.algolia.listeningRefs) {
          firebaseSearch.algolia.listeningRefs.child_added.off(
            'child_added',
            firebaseSearch.algolia.handlers.child_added
          );
          firebaseSearch.algolia.listeningRefs.child_changed.off(
            'child_changed',
            firebaseSearch.algolia.handlers.child_changed
          );
          firebaseSearch.algolia.listeningRefs.child_removed.off(
            'child_removed',
            firebaseSearch.algolia.handlers.child_removed
          );
        } else {
